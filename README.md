...
- 🗺️ **交互式人迹地缘迁徙地图 (V1.9)**：
  - **历史坐标图谱**：基于 Leaflet.js 地图渲染引擎，将谱书中所有记录了经纬度坐标的“出生地”、“常住地”、“安葬地”标注在地图上。
  - **地理迁徙矢量路线**：全自动绘制并追踪先祖自出生到迁出、再到终老安葬的代际/生命轨迹流线（男性采用蓝色线，女性采用粉色虚线），支持按“分支”或“特定成员”进行高阶过滤筛选。
  - **馆藏墨宝 Sepia 滤镜**：专门为地图图层添加了符合古雅质感的宣纸/羊皮纸 sepia 复古滤镜，与修谱的传统文献格调完美协调。
- 📖 **字辈昭穆智能校验器 (V1.8)**：
  - **字辈诗在线解析**：支持在字辈控制台中直接输入用空格或标点分隔的谱系字辈诗（例如 `守 浩 绍 中 德 承 先 启 后 贤`），系统将一键解析并注册各世代的辈序规范。
  - **宗族代际智能扫描**：内置强大的逻辑校验扫描仪，提供高亮问题反馈：
    - *字辈错配*：检查族人名字或录入辈字是否与其世代应有辈字冲突。
    - *同胞不一致*：检测同父母的同胞兄弟姐妹是否被错误地录入到了不同的代数或拥有不同的辈字。
    - *世代断链跳代*：审计父母世代数与子代世代数之间是否异常断开跳代（即相差非标准 1 代）。
- 📜 **文献考证与学术引用 (V1.7)**：
  - **宗谱考证文献登记**：提供独立的文献数据登记控制台，支持记录谱籍、墓志铭、官方历史文献、手稿等来源的朝代、作者、卷册及引文链接。
  - **证据主张关联比对**：支持在族人详情页为姓名、生平、生卒日期等特定字段绑定引文依据，记录 verbatim 原文引用和现代学术考释。
  - **可信度与标号链接**：自动为被佐证字段附加超链接上标标号（如 `[1]`），支持 smooth-scroll 悬浮跳转，辅助建立严谨的学术化谱系档案。
- 📊 **统计看板与迁徙分析 (V1.6)**：
  - **宗族数据统计看板**：自动计算谱系成员数、平均寿命、世代繁衍分布柱状图、Y-DNA/mtDNA 单倍群占比等，以精美直观的图表卡片呈现。
  - **人迹地缘迁徙时空轴**：跨项目提取并按年代排序梳理出“出生落户”、“寿终过世”以及“安葬地”等关键事件，拼接成迁徙历史时间轴。
  - **世系扇形图 (Fan Chart)**：新增精美的圆形径向世系扇形图，以指定先祖为中心逐层向外延展后代，支持动态 Hover 高亮与卡片点击。
- 🏮 **中式竖排世系与打印排版 (V1.5)**：
