      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-950 p-6 rounded-2xl text-white shadow-xl">
        <div>
          <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight bg-gradient-to-r from-amber-200 via-yellow-100 to-white bg-clip-text text-transparent">
            📜 {projects.length > 0 ? t("landingTitle") : (language === "zh" ? "开启您的数字化家谱" : "Start Your Genealogy")}
          </h2>
        ...
        >
          {isCreating ? t("cancelBtnX") : t("newPedigreeBook")}
        </button>
      </div>
      ...
      {isCreating && (
        <form onSubmit={handleCreateProject} className="bg-white/80 backdrop-blur-md p-6 rounded-2xl border border-slate-200/80 shadow-lg space-y-4 animate-slide-up">
          <div className="border-b border-slate-100 pb-3">
            ...
            <p className="text-xs text-slate-500">{t("createNewRepoDesc")}</p>
          </div>
          <div className="grid grid-cols-1 gap-4">
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1">{t("projectName")}</label>
              <input 
                type="text" 
                required 
                placeholder={language === "zh" ? "例如：张氏大成宗谱 · 江南支派" : "e.g., Zhang Clan Genealogy - Jiangnan Branch"}
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-1">{t("projectDesc")}</label>
              <textarea 
                placeholder={language === "zh" ? "描述本房支的源流、始祖迁徙、编纂信息等..." : "Describe the migration origin, branch founder, or compiler info..."}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {isLoading ? (
          <div className="col-span-full flex flex-col items-center justify-center py-16 text-slate-400 bg-white rounded-2xl border border-slate-100 shadow-sm">
            ...
            <p className="text-sm">{language === "zh" ? "正在连接家谱数据库..." : "Accessing genealogy databases..."}</p>
          </div>
        ) : error ? (
          ...
          <div className="col-span-full text-center py-16 text-slate-400 bg-white rounded-2xl border-2 border-dashed border-slate-200 flex flex-col items-center justify-center">
            ...
            <p className="font-medium text-slate-600">{language === "zh" ? "尚无已初始化的家谱册书。" : "No Pedigree books initialized yet."}</p>
            <p className="text-xs text-slate-400 mt-1">{language === "zh" ? "点击右上角录入新谱书开始编纂。" : "Click \"New Pedigree Book\" on the top right to start compiling."}</p>
          </div>
        ) : (
          projects.map((p) => (
            <Link key={p.id} href={`/projects/${p.id}`} className="block group">
              <div className="bg-white border border-slate-100 hover:border-amber-200 rounded-2xl p-6 shadow-sm hover:shadow-xl group-hover:-translate-y-1.5 transition-all duration-300 h-full flex flex-col relative overflow-hidden">
                ...
                <div className="flex items-start justify-between mb-3">
                  <h3 className="font-bold text-lg text-slate-900 group-hover:text-amber-800 transition-colors line-clamp-1 font-serif">
                    {p.name}
                  </h3>
                  <span className="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full bg-slate-50 text-slate-500 border border-slate-100 font-sans">
                    📜 {p.projectType === "FAMILY" ? (language === "zh" ? "宗族" : "Clan") : p.projectType}
                  </span>
                </div>

                <p className="text-slate-500 text-sm mt-1 flex-1 line-clamp-3 leading-relaxed font-sans">
                  {p.description || t("noDescription")}
                </p>

                <div className="border-t border-slate-100 pt-4 mt-5 flex items-center justify-between text-xs text-slate-400 font-sans">
